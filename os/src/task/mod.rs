...
impl TaskManager {
    ...
    /// Increment the syscall count of current task
    fn incr_current_task_syscall_times(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].incr_syscall_times(syscall_id);
    }
    /// Get the current task's syscall count
...
pub fn increase_syscall_times(syscall_id: usize) {
    TASK_MANAGER.incr_current_task_syscall_times(syscall_id);
}
